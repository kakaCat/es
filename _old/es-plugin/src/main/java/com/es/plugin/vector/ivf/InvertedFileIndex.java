...
public class InvertedFileIndex implements Serializable {
    ...
    // GPU加速组件（transient不序列化）
    private transient GPUVectorSimilarity gpuSimilarity;
    ...
    private void initializeGPU() {
        try {
            ...
            if (gpuSimilarity.isGPUAvailable()) {
                System.out.println("GPU acceleration enabled for IVF index");
            } else {
                System.out.println("GPU not available, using CPU for vector computations");
            }
        } catch (Exception e) {
            System.err.println("Failed to initialize GPU: " + e.getMessage());
            this.gpuSimilarity = null;
        }
    }
    ...
    public void train(float[][] trainingVectors) {
        ...
        // 尝试使用GPU加速训练
        if (gpuSimilarity != null && gpuSimilarity.isGPUAvailable() && trainingVectors.length > 1000) {
            try {
                System.out.println("Using GPU-accelerated KMeans training");
                GPUKMeansTrainer gpuTrainer = new GPUKMeansTrainer(nlist);
                this.centroids = gpuTrainer.train(trainingVectors);
                gpuTrainer.cleanup();
            } catch (Exception e) {
                System.err.println("GPU training failed: " + e.getMessage());
                System.out.println("Falling back to CPU training");
                SimpleKMeansTrainer cpuTrainer = new SimpleKMeansTrainer(nlist);
                this.centroids = cpuTrainer.train(trainingVectors);
            }
        } else {
            // CPU训练
            System.out.println("Using CPU KMeans training");
            SimpleKMeansTrainer cpuTrainer = new SimpleKMeansTrainer(nlist);
            this.centroids = cpuTrainer.train(trainingVectors);
        }
    ...
    public List<SearchResult> search(float[] queryVector, int k, int nprobe) {
        ...
        // 使用GPU批量计算（如果候选向量数量足够大）
        if (gpuSimilarity != null && gpuSimilarity.isGPUAvailable() && candidates.size() > 64) {
            try {
                // 准备候选向量矩阵
                float[][] candidateVectors = new float[candidates.size()][];
                for (int i = 0; i < candidates.size(); i++) {
                    candidateVectors[i] = candidates.get(i).vector;
                }

                // GPU批量计算
                float[][] queryMatrix = new float[][]{queryVector};
                float[][] scoresMatrix = null;

                switch (metricType) {
                    case "l2":
                        scoresMatrix = gpuSimilarity.batchL2Distance(queryMatrix, candidateVectors);
                        break;
                    case "cosine":
                        scoresMatrix = gpuSimilarity.batchCosineSimilarity(queryMatrix, candidateVectors);
                        break;
                    case "dot":
                        scoresMatrix = gpuSimilarity.batchDotProduct(queryMatrix, candidateVectors);
                        break;
                }

                if (scoresMatrix != null && scoresMatrix.length > 0) {
                    float[] scores = scoresMatrix[0];
                    for (int i = 0; i < candidates.size(); i++) {
                        VectorDoc doc = candidates.get(i);
                        results.add(new SearchResult(doc.docId, scores[i], doc.metadata));
                    }
                } else {
                    throw new Exception("GPU computation returned null");
                }
            } catch (Exception e) {
                System.err.println("GPU batch scoring failed, falling back to CPU: " + e.getMessage());
                // 降级到CPU
                results.clear();
    ...
    private int[] findNearestClusters(float[] vector, int nprobe) {
        ...
        // 尝试使用GPU批量计算
        if (gpuSimilarity != null && gpuSimilarity.isGPUAvailable() && nlist > 32) {
            try {
                float[][] queryMatrix = new float[][]{vector};
                float[][] distanceMatrix = null;

                switch (metricType) {
                    case "l2":
                        distanceMatrix = gpuSimilarity.batchL2Distance(queryMatrix, centroids);
                        break;
                    case "cosine":
                        distanceMatrix = gpuSimilarity.batchCosineSimilarity(queryMatrix, centroids);
                        break;
                    case "dot":
                        distanceMatrix = gpuSimilarity.batchDotProduct(queryMatrix, centroids);
                        break;
                }

                if (distanceMatrix != null && distanceMatrix.length > 0) {
                    scores = distanceMatrix[0];
                }
            } catch (Exception e) {
                System.err.println("GPU cluster search failed, falling back to CPU: " + e.getMessage());
                // 降级到CPU
                for (int i = 0; i < nlist; i++) {
